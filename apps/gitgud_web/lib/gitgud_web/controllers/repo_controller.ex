  ...
  def index(conn, %{"user_login" => user_login} = _params) do
    if user = UserQuery.by_login(user_login, preload: [:public_email, :repos], viewer: current_user(conn)) do
      ...
      site_stats = %{
        ...
        first_at: UserQuery.first_at()
      }
      render(conn, "index.html", user: user, stats: Map.new(user.repos, &{&1.id, stats(&1, batch_stats)}), site_stats: site_stats, earliest: Repo.earliest)
    end || {:error, :not_found}
  end
