export default () => {
  const field = document.getElementById("quick-setup")
  if(field) {
    ...
    let quickSetupContentRepoUrls = field.querySelectorAll(".content .repo-url")
    ...
    httpCloneButton.addEventListener("click", event => {
      ...
      quickSetupContentRepoUrls.forEach(repoUrl => repoUrl.innerHTML = httpCloneInput.value)
      if(sshCloneButton) {
    ...
    if(sshCloneButton) {
      sshCloneButton.addEventListener("click", event => {
        ...
        quickSetupContentRepoUrls.forEach(repoUrl => repoUrl.innerHTML = sshCloneInput.value)
